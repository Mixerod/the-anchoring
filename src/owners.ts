...
/**
 * Markers are split by target format, because the target formats disagree.
 *
 * In a `CODEOWNERS` file only `#` begins a comment. An HTML comment on its own line is
 * parsed as a pattern with two malformed owners, and GitHub's CODEOWNERS validator
 * reports a syntax error — so the generated block that was supposed to be invisible is
 * the one thing in the file that breaks. `AGENTS.md` keeps the HTML pair: it is Markdown,
 * where `#` is a heading.
 *
 * The HTML pair below is retained only so a `CODEOWNERS` written by an earlier version is
 * recognised and rewritten once. It must never be emitted into a `CODEOWNERS` again.
 */
export const OWNERS_START_MARKER = '# kb:owners:start'
export const OWNERS_END_MARKER = '# kb:owners:end'

export const LEGACY_OWNERS_START_MARKER = '<!-- kb:owners:start -->'
export const LEGACY_OWNERS_END_MARKER = '<!-- kb:owners:end -->'
...
export function renderCodeowners(
  ...
  // The current pair first, then the legacy HTML pair. A file written by an earlier
  // version is migrated in place: the block between the old markers is replaced by the
  // new block, and every hand-written line outside it survives untouched. Because the
  // new content differs, `--check` reports `stale` first and the migration happens once.
  for (const [start, end] of [
    [OWNERS_START_MARKER, OWNERS_END_MARKER],
    [LEGACY_OWNERS_START_MARKER, LEGACY_OWNERS_END_MARKER],
  ] as const) {
